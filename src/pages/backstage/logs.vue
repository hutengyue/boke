    ...
    <el-card class="user-card" shadow="hover">
      <template #header>
        <div class="card-header">
          ...
          <div class="actions">
            <el-button type="primary" :icon="Plus" @click="data.dialog = true;data.type = 1;">新增日志</el-button>
            <el-button type="danger" :icon="Delete" :disabled="data.selectLogs.length==0" @click="del()">批量删除</el-button>
          </div>
        </div>
      </template>
      <el-table :data="data.list" style="width: 100%" :border="false" :stripe="true"
        ...
        <el-table-column fixed prop="logId" label="编号" min-width="5%" />
        <el-table-column label="日志内容" min-width="50%">
...
async function del(row) {
  try {
    ...
    await proxy.$http.post('/log/delete', { logId: row ? row.logId : data.selectLogs.map(item => item.logId) })
...
async function add() {
  try {
    const url = data.type === 1 ? '/log/add' : '/log/update'
